...
subprocess.run("sudo bash ./dependencies/noisebox.sh", shell=True)
...
copy("./rpi_config/hosts", "/etc/hosts")
...
subprocess.run("sudo cp ./rpi_config/noisebox.service /lib/systemd/system/noisebox.service", shell=True)
...
copy("./rpi_config/config.txt", "/boot")
copy("./rpi_config/profile", "/etc/profile")
...
copy("./rpi_config/modules", "/etc/modules")

# VPN

if os.path.isfile('./custom_config/vpncloud/vpncloud.txt'):

    print(green + 'Configuring vpncloud for remote access...' + res)

    subprocess.run("sudo bash ./dependencies/vpncloud.sh", shell=True)
    copy("/etc/vpncloud/example.net.disabled", "/etc/vpncloud/mynet.net")
    f = open(file='./custom_config/vpncloud/vpncloud.txt', mode='r')
    ...
    f = open(file='./custom_config/vpncloud/hosts.txt', mode='r')
...
if os.path.isfile('./custom_config/telegraf/telegraf.conf'):

    print(green + 'Configuring telegraf metrics server...' + res)

    subprocess.run("sudo bash dependencies/telegraf.sh", shell=True)
    ...
    copy("./custom_config/telegraf/telegraf.conf", "/etc/telegraf/telegraf.conf")
else:
